       ...
       <TouchableOpacity onPress={() => navigation.navigate}>
        ...
        {learnCards.map((card, index) => (
          ...
          <View key={index} style={styles.learnCard}>
            <Text style={{ color: '#ef4565', marginBottom: 10, textTransform: 'capitalize', fontSize: 16 }}>{card.subject}</Text>
            <Text style={{ color: '#d8eefe', fontFamily: 'Nunito', fontSize: 18, fontWeight: '700', lineHeight: 25 }}>{card.topic.length > 47 ? `${card.topic.substring(0, 47)}...` : card.topic}</Text>
            <View style={{ flexDirection: 'row', marginTop: 17, marginBottom: 15 }}>
              <Image source={{ uri: card.createdBy.photo }} width={20} height={20} style={{borderRadius:20 ,}} />
               <Text style={{color:'#d8eefe'}}>{"  "}{card.createdBy.userName}</Text>
            </View>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' , }}>
              <View style={{ flexDirection: 'row', alignItems:'center',  }}>
                <SvgImgInterested/>
                {/* <Text>{card.interestedStudents} Interested</Text> */}
                <Text style={{color:'#d8eefe'}}> {" "}{card.interestedStudents.length} Interested</Text>
              </View>
              <View style={{ flexDirection: 'row' , }}>
               
                <Text style={{color:'#FFF'}}>Due - {new Date(card.dueDate).toLocaleDateString()}</Text>
              </View>
            </View>
           </View> 
