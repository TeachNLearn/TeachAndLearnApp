...
const FilterSearch = () => {
  ...
  const filterSearch = async () => {
    setIsLoading(true)    
      await axios
        .post(`${BASE_URL1}${apiVersion}/user/filter?standard=${standard}&subject=${subject}&programme=${programme}&preferredLanguage=${prefferedLanguage}`,{}, {
  ...
  <>
    ...
    <Pressable onPress={()=>{setStandard(''),setSubject(''),setPrefferedLanguage(''),setOpenDropDown(false)}}>
        <Text style={{marginRight:20}}>clear all</Text>
      </Pressable>
    <View style={{padding:20,flexDirection:'row',justifyContent:'space-between',width:'100%'}}>
    
      {
        AllText.map((e:any,i:number)=>{
          return (
            <DropDownComponent key={i} onPress={()=>setDD(e)} heading={e} text={e === 'Subject' ?subject:e==='Class'?standard:e==='Language'?prefferedLanguage:''}/>
          )
        })
      }
    </View>
     ...
     {
      aboutt=== 'Subject'? <DropDownBox setOpenDD={setOpenDropDown} setField={setSubject} showDropDown={openDropDown} aboutt={aboutt}/>:aboutt === 'Class'? <DropDownBox setOpenDD={setOpenDropDown} showDropDown={openDropDown} setField={setStandard} aboutt={aboutt}/>:aboutt ==='Language'? <DropDownBox setField={setPrefferedLanguage} setOpenDD={setOpenDropDown} showDropDown={openDropDown} aboutt={aboutt}/>:null
     }
